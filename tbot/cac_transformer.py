...
from ssg.build_yaml import Rule
...
def get_profile_params(root: str, product: str, profile_id: str) -> Dict[str, Any]:
    """
    Get profile variables. Example:
    {
        'var_event_record_qps': {'ocp4': {'high-rev-4': '50'}},
        'var_oauth_inactivity_timeout': {'ocp4': {'high-rev-4': '10m0s'}},
        ...
    }
    """
...
class RulesTransformer:
    ...
    def _get_params(self, root: str, rule_yaml: Rule, rule_obj: RuleInfo) -> None:
        """Get params related to this rule and update rule object."""
        if not rule_yaml.is_templated():
            return
        product_yml_path = product_yaml_path(root, self.product)
        product = load_product_yaml(product_yml_path)
        xccdf_variable = rule_yaml.get_template_vars(product).get(XCCDF_VARIABLE)

        if xccdf_variable:
            param_id = xccdf_variable
            if param_id in self._params_by_id:
                param_obj = self._params_by_id[param_id]
                rule_obj.add_parameter(param_obj)
            elif param_id in self.profile_params:
                param_data = self.profile_params[param_id]
                ...
                param_obj.set_options(get_variable_options(root, param_id))
                self._params_by_id[param_id] = param_obj
                rule_obj.add_parameter(param_obj)
            else:
                logger.debug(f"{param_id} in profile variables not found.")
        else:
            logger.debug(f"{XCCDF_VARIABLE} in rule {rule_obj.id} not found.")
    ...
    def add_rules(self, rules: List[str]) -> None:
        ...
        if len(rule_errors) > 0:
            ...
            logger.error(f"Error loading rules: \n{', '.join(rule_errors)}")
    ...
    def _add_rule(self, rule_id: str) -> Optional[str]:
        ...
        try:
            ...
            return f"Error in processing rule {rule_id}: {e}"
    ...
    def _load_rule_yaml(self, rule_obj: RuleInfo) -> None:
        ...
        try:
            rule_yaml = Rule.from_yaml(rule_file)
        except Exception as e:
            raise RuntimeError(f"Failed to parse {rule_file}: {str(e)}")
        ...
        self._get_params(self.root, rule_yaml, rule_obj)
